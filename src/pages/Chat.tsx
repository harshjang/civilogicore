...
export default function Chat() {
  ...
  const sendMessage = async (text?: string) => {
    ...
    // Simulated response (replace with AI gateway when Cloud is enabled)
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
      ...
      <div className="p-6 border-b border-border bg-card/50">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-mono font-bold text-foreground">AI Assistant</h1>
              <p className="text-xs text-muted-foreground font-mono">CIVIL ENGINEERING · GEOSPATIAL · STRUCTURAL</p>
            </div>
          </div>
        </motion.div>
      </div>
        ...
        {messages.length === 0 && (
          ...
          >
            <div className="text-center">
              ...
              <h2 className="font-mono text-lg font-semibold text-foreground">Civil Engineering Agent</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-xl w-full">
              {suggestions.map((s) => (
                <button
                  ...
                  className="p-3 rounded-lg border border-border bg-card text-left text-sm text-foreground hover:border-primary/30 transition-colors font-mono"
                >
                  {s}
                </button>
              ))}
        ...
        {isLoading && (
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-md bg-primary/10 border border-primary/30 flex items-center justify-center">
              <Bot className="w-4 h-4 text-primary animate-pulse-cyan" />
            </div>
